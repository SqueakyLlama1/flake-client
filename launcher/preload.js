...
contextBridge.exposeInMainWorld('flakeAPI', {
    ...
    updateRPC: (details, state) => ipcRenderer.invoke('update-rpc', details, state),
    checkRPC: () => ipcRenderer.invoke('check-rpc'),
    ...
    onRPC: (callback) => {
        ...
        ipcRenderer.on('rpc-status', subscription);
        
        return () => {
            ipcRenderer.removeListener('rpc-status', subscription);
        };
    }
});